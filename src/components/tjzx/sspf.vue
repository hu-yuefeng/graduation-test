      ...
      <el-tabs type="border-card">
        <el-tab-pane label="数据表展示">
          ...
          <el-table
            ...
            border
            height="380px"
            :default-sort="{prop: 'health',prop: 'discipline',prop: 'dormitoryculture',prop: 'total', order: 'descending'}"
          >
            <el-table-column label="楼宇号" prop="lynumber" width="90px"></el-table-column>
            <el-table-column label="宿舍号" prop="dom" width="60px"></el-table-column>
            <el-table-column label="学院" prop="campus" width="100px"></el-table-column>
            <el-table-column label="班级" prop="class" width="90px"></el-table-column>
...
export default {
  ...
  methods: {
    shuju(){
      this.cunzhu.forEach(item => {
        console.log(item);
        this.sushe.push(`${item.lynumber}-${item.dom}`)
        this.health.push(item.ws)
        this.discipline.push(item.jl)
        this.culture.push(item.wh)
        // _this.health = response.data[0].ws;
        // _this.discipline = response.data[0].jl;
        // _this.culture = response.data[0].wh;
      });
    },
    getAll(){
      var _this = this;
