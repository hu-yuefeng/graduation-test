        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="宿舍号" prop="sushe">
              <el-input
                v-model="searchForm.sushe"
                placeholder="请输入您所在的宿舍号"
                clearable
              ></el-input>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      const that = this;
      this.$http.post("http://localhost/dfcx/search", { data }).then((res) => {
        if (res.data.length === 1) {
          this.searchResult = true;
          // console.log(res.data);
          that.resultForm.lyNumber = res.data[0].lynumber;
          that.resultForm.sushe = res.data[0].dom;
